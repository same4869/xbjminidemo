...
class PluginBuilder():
    ...
    def parseManifestFile(self):
        ...
        found = re.search(r'package="(com\.example\.xunwang\.[^"]+)"', contents)
        if not found:
    ...
    def parseBuildProp(self):
        ...
        includeSrcDir = 'src/main/java/com/example/xunwang/' + pluginPackageName
    ...
    def proguard(self):
        ...
        cmd = ' -jar ' + self.sdkDir + '/tools/proguard/lib/proguard.jar ' \
              ...
              + ' @proguard-rules.pro' \
              + ' -dontnote ' \
              + ' -keep class com.example.xunwang.' + pluginPackageName + '.LauncherPlugin'
